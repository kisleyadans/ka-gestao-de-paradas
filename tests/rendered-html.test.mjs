...
test("inclui a sincronização gratuita e concorrente no painel publicado", async () => {
  ...
  assert.match(sync, /ka_editor_lock/);
  assert.match(sync, /ka_editor_presence/);
  assert.match(sync, /editorSessionId/);
  assert.match(sync, /acquireEditorLease/);
  assert.match(sync, /baseState: sharedPart\(currentShared\)/);
  assert.doesNotMatch(sync, /mergeSharedState,/);
  ...
  assert.match(sync, /andamento por/);
  assert.match(sync, /Sair Admin/);
  assert.match(rules, /hasActiveEditorSession/);
  assert.match(rules, /match \/ka_editor_presence\/current/);
  assert.match(rules, /allow read: if true/);
  assert.match(rules, /request\.resource\.data\.editorSessionId == editor\.data\.sessionId/);
